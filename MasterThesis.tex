...
In the case where $\mathcal{Y} = \mathbb{R}$ the problem is said to a regression problem, e.g. predicting the price of a house. On the contrary, if $\mathcal{Y}$ is binary such that $\mathcal{Y} = \{0, 1\}$ then the problem is said to be a (binary) classification problem\sidenote{If $\mathcal{Y}\in \mathbb{Z}$ then it would be a multiclass classification problem.}. 

Without any loss of loss of generality let the focus for now be on 
