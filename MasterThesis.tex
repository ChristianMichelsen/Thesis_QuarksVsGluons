...
\begin{marginfigure}
  \includegraphics[width=0.98\textwidth]{figures/cross_validation/kfold.pdf}
...
\begin{marginfigure}
  \includegraphics[width=0.98\textwidth]{figures/cross_validation/kfold_time.pdf}
...
\begin{marginfigure}[-3cm]
  \includegraphics[width=0.98\textwidth]{figures/decision_tree/tree_example.pdf}%     without .tex extension
  % or use \input{mytikz}
...
\begin{figure}
  \centering
  \includestandalone[width=0.7\textwidth]{figures/decision_tree/decision_tree}%     without .tex extension
  % or use \input{mytikz}
...
\begin{marginfigure}
  ...
  \caption[Random Search]{Visualization of the difference between grid search and random search. Taken from \citet{bergstraRandomSearchHyperparameter2012}}
...
\begin{equation}
  \label{eq:ml:shap_feature_importance}
    \phi_i = \sum_{S \subseteq \tilde{N} \backslash \{i\}} \frac{|S|!(N-|S|-1)!}{N!} \left[ f_x(S \cup \{i\}) - f_x(S) \right] ,
\end{equation}
which can be simplified to:
\begin{equation}
  \label{eq:ml:shap_feature_importance_simplification}
  \begin{split}
    \phi_i        &= \sum_{S \subseteq \tilde{N} \backslash \{i\}} k(S) \cdot \Delta_{f_x}(S) \\
    k(S)             &\equiv \frac{|S|!(N-|S|-1)!}{N!} \\
    \Delta_{f_x}(S)  &\equiv \left[ f_x(S \cup \{i\}) - f_x(S) \right].
  \end{split}
\end{equation}


In equation \eqref{eq:ml:shap_feature_importance} $\tilde{N}$ is the set of all input features\sidenote{compared to $N=|\tilde{N}|$ which is the number of all input featues}, $S \subseteq \tilde{N} \backslash \{i\}$ means a subset $S$ of $\tilde{N}$ without feature $i$, $S \cup \{i\}$ means the set $S$ with feature $i$ and $f_x(S) = f(h_x(z'))$ where $h_x(z')$ is the mapping function from the binary $z'$ space to the input feature space $x$.In equation \eqref{eq:ml:shap_feature_importance_simplification} the function is simplified to its basic constituents: the difference in performance  between including feature $i$ and not including it, $\Delta_{f_x}$, and its weight $k$. 

To get a better understanding of the different sets in the summation, one could look at the decision tree shown in Figure~\ref{fig:ml:decision_tree}. Here $\tilde{N}$ would be $\tilde{N}=\{p_\perp, E_\mathrm{jet} \}$. For the feature $i=p_\perp $ one would thus have:
\begin{equation}
  \begin{split}
    \phi_{p_\vert} = & \sum_{S \subseteq \tilde{N} \backslash \{p_\perp \}} k(S)  \cdot\Delta_{f_x}(S)  \\
                   = & \sum_{S \in \left[\{\}, \{E_\mathrm{jet}\} \right]} k(S)  \cdot\Delta_{f_x}(S)  \\
                   = & \frac{0! (2-0-1)!}{2!}  [ f_x(\{p_\perp\}) - f_x(\{\})]  \\ 
                     & +    \frac{1! (2-1-1)!}{2!}  [f_x(\{ E_\mathrm{jet}, p_\perp \}) - f_x(\{ E_\mathrm{jet} \})].
  \end{split}
\end{equation}
Whereas $k(S)$ are easily calculated, $\Delta_{f_x}$ depends on the data. As the number of features grows, the number of terms in the sum grows exponentially. What \citet{lundbergConsistentIndividualizedFeature2019} did was to develop an algorithm that could solve this for trees in polynomial time.
