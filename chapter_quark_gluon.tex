...
\end{equation}
where $z_i \equiv E_i / E_\mathrm{jet}$ is a momentum fraction, i.e. $0 \leq z_i \leq 1$, $\theta_i \equiv \Omega_i / R$ is a normalized angle with respect to the jet axis where the $R$ is the jet radius such that $0 \leq \theta_i \leq 1$, and $i$ runs over all the jet constituents \citep{grasSystematicsQuarkGluon2017,larkoskiGainingMutualInformation2014}. Different values of ($\beta, \kappa$) probe different aspects of the (gluon) jet fragmentation, where we will limit the analysis to the five sets of ($\beta, \kappa$)-values shown in Figure~\ref{fig:q:LHA}, where each of the sets of variables are related to the following aspects:
...
The distribution of $\lambda_0^2$, related to the transverse momentum distribution, in gluon events is seen in Figure~\ref{fig:q:generalized_angularities_cha_lambda_0_2}. Here the distribution of $\lambda_0^2$ is shown for MC Truth (actual gluons using truth-label), MC selected (gluons selected using the $g$-tag) and Data (gluons selected using the $g$-tag). The generalized angularities are computed for both charged and neutral jets, where this figure shows the charged jets. The MC Selected has been scaled to Data according to the fraction of the number of events in each: $w_\mathrm{MC} = N_\mathrm{Data} / N_\mathrm{MC}$. The MC Truth has been scaled with the same weight multiplied with the gluon efficiency $w_\mathrm{MC-Truth} = w_\mathrm{MC} \cdot \varepsilon_g^{3\dash\mathrm{jet}}$. The $\lambda_\beta^\kappa=0$ values has been removed before plotting for all sets of values of $(\beta, \kappa)$. 

In Figure~\ref{fig:q:generalized_angularities_cha_lambda_0_2} it is seen how MC Selected and Data distributions matches each other quite well. That the MC Truth and Data not matches equally well indicates that XXX \TODO. The rest of the plots can be seen in Figure~\ref{fig:q:generalized_angularities_cha_lambda_0_2}--\ref{fig:q:generalized_angularities_neu_lambda_0_0}.
...
XXX Intro here, 4 jet blab \TODO

Figure \ref{fig:q:gtag_scores_3j_sig_bkg}, the $\gamma_\mathrm{tag}$ cut off threshold is set to $\gamma_\mathrm{cutoff} = 0.8$ for 4-jet events and to $\gamma_\mathrm{cutoff} = 0.9$ for 3-jet events. This corresponds to selecting \num{41117} 4-jet events in MC as gluon events with a signal efficiency of $\varepsilon_g^{4\dash\mathrm{jet}} = \SI{23.30}{\percent}$ and a signal purity is $\rho_g^{4\dash\mathrm{jet}}=\SI{92.67}{\percent}$
...
\begin{enumerate}[leftmargin=*,labelindent=40pt]
  ...
  \item[$E_\mathrm{diff}$:] The relative difference in energy between the gluon with the highest and lowest jet energy: $E_\mathrm{diff} = \frac{E_\mathrm{max}-E_\mathrm{min}}{E_\mathrm{max}+E_\mathrm{min}}$.
  \item[$E_{\mathrm{rel}_\mathrm{min}}$:] The relative energy of the gluon with the lowest jet energy and the sum: $E_{\mathrm{rel}_\mathrm{min}} = \frac{E_\mathrm{min}}{E_\mathrm{max}+E_\mathrm{min}}$.
  \item[$E_\mathrm{rel}$:] The relative energy of the gluon with the lowest jet energy and the highest jet energy: $E_\mathrm{rel} = \frac{E_\mathrm{min}}{E_\mathrm{max}}$.
  ...
  \item[$\ln \left( k_T^2 / m_\mathrm{vis}^2 \right)$:] Logarithm of the ratio between the $k_T^2$ and the visible mass (squared).  
  ...
  \begin{fullwidth}
    ...
    \end{flalign}
  \end{fullwidth}
  where $\widetilde{m}^2 = m^2 \cdot m_Z^2 / m_\mathrm{vis}^2$.  

  \item[] \textbf{Jet Clustering} 
  \item[$R_{gg}^\mathrm{kT}$:]  
  \item[$R_{gg}^\mathrm{CA}$:]  
\end{enumerate}


% d_variable_nicefy = {'E_diff': r"$E_\mathrm{diff}$", 
%                      'E_min_rel': r"$E_{\mathrm{rel}_\mathrm{min}}$", 
%                      'E_rel': r"$E_\mathrm{rel}$", 
%                      'delta_theta': r"$\Delta_\theta$", 
%                      'm_gg': r"$m_{gg}$", 
%                      'phi_planes': r"$\phi_\mathrm{\parallel}$", 
%                      'ln_kT_gg_m_viz': r"$\ln \left( k_T^2 / m_\mathrm{vis}^2 \right)$", 
%                      'pT_antenna2': r"$p^2_{\perp,\mathrm{A}}$", 
%                      'R_gg_kt': r"$R_{gg}^\mathrm{kt}$", 
%                      'R_gg_CA': r"$R_{gg}^\mathrm{CA}$",  
%                      'eff_E_2_max_E': r"$\varepsilon_{E_\mathrm{max}}$", 
%                      'eff_E_2_max_b': r"$\varepsilon_{b_\mathrm{max}}$",
%                      }
