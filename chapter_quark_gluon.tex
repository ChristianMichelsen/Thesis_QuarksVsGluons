...
\end{equation}
This is only a pseudo efficiency since this number is based on the jets in the event and not the event itself, however, by plotting it as a function of an event variable and comparing MC to Data, the validity of the $g$\=/tagging algorithm can be gauged. The first of the event variables used is the $g$\=/tag of the event, see Figure~\ref{fig:q:effiency_btag_bbg_gtag}. 
...
To measure how gluon jets hadronize, i.e. their jet distributions, the \emph{generalized angularities} provide an overall framework for doing so. The generalized angularities are a two-parameter family of variables depending on the angular weighting $\beta \geq 0$ and an energy weighting factor $\kappa \geq 0$:
\begin{equation}
...
The generalized angularity distributions for gluons will be measured in 3-jet events since this provide a (relatively) clean sample of gluon jets. 
...
\marginnote{Note that the number of selected events in the 4-jet case is much lower than in the 3-jet case and that the signal purity is lower as well, $\rho_g^{3\dash\mathrm{jet}}=\SI{98.77}{\percent}$ vs. $\rho_g^{4\dash\mathrm{jet}}=\SI{92.67}{\percent}$, at the selected cut value.}
...
\begin{enumerate}[leftmargin=*,labelindent=40pt]
  ...
  \item[$p^2_{\perp,\mathrm{A}}$:] The $p_\perp$ antenna defined as: 
  \begin{fullwidth}
    \begin{flalign}
...
% The observables have uncertainties which are dominantly from the uncertainty in the jet energy measurement. This has been measured by ALEPH to be eq. (XXX 0.57). The angular uncertainties are in comparison small and so to a good approximation one can determine the observalbe uncertainty by propagating from eq. (0.57). Hweover, this yields different uncertainties for same size observables and so one can not give a single number. Below is an example for Ediff where the distribution of the uncertanty on Ediff and its relative uncertainty are shown. As can be seen, it is of order $10\%$, and this is expected to be the largest. These uncertainties could be incorporeated into the results by unfolding, however this remains to be future work. 
