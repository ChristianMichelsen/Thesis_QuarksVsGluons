...
%   \vspace{\abovecaptionskip}
% \end{figure}
For the half-life and the choice of objective function these can be seen in Figure~\ref{fig:h:hpo_gridsearch_objective} and \ref{fig:h:hpo_gridsearch_halflife}. In the first of the two figures is easily seen that even though $T_{\frac{1}{2}} = \SI{20}{\years}$ is the minimum, the uncertainties are so large that nothing can be concluded definitively with regards to the half-life parameter related to the weights $w(t)$. In contrary, in the second figure there is a clear performance difference between the different loss functions: not only does the Cauchy loss $\ell_\mathrm{Cauchy}$ archive the best value of $f_\mathrm{eval}$.
